...
def training_pipeline(config: PipelineConfig) -> EvaluationSummary:
    ...
    split_output = split_step(data=raw_data, config=config.split)
    train_data = get_train_data(split_output=split_output)
    test_data = get_test_data(split_output=split_output)

    # Preprocess training data only for downstream steps
    preprocessing_output = preprocessing_step(data=train_data, config=config.preprocessing)
